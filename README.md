...
任务解析等 Host 辅助功能通过同进程 `spendAccounting.recordUsage(principal, call)` 将最终 Token 用量记入同一持久账本，并采用相同模型价格。调用方必须先完成账号与模型权限检查，传入 Host 认证的账号和稳定请求标识；重试记账不会重复扣费，账本不接受用量数据替换该账号。
...
三条约束决定了它不会把已经核对正确的表改坏：

| 约束 | 行为 |
|---|---|
| **只向前** | 每次观测写成一个 `effectiveAt = 抓取时刻` 的新 phase，绝不改既有 phase。每次调用仍按自身时间戳选表，**已计价的历史不重算** |
| **不覆盖已核过的价** | 默认 `fillUnpricedOnly: true`，只给知识库和手工改价都没覆盖的模型建行。想让某家完全跟目录走，把它加进 `providers` 白名单 |
| **手工永远优先** | 该 `(provider, model)` 存在管理员改价时，同步整体跳过 |

为什么公开目录不能直接当权威——2026-09-11 实测两条：它把 DeepSeek 的**人民币标价按自己的汇率**（约 6.67）折成美元，与部署配置的 7.2 差 8%；且它对 `deepseek-v4-pro` 仍是 2026-08-17 之前的旧表，既无 09-10 降价也无分时段。**人民币原生计价的供应商不要放进白名单。**

目录的 `utc_days` + `utc_start/utc_end` 窗口会映射成 `peakDays`/`peakHours`（UTC+8 → 北京时间），所以分时段定价也能同步。只接受「两档 + 整点 + 不跨午夜 + 各窗口同一组星期」的形态；其余（例如长上下文加价那种没有时间窗的 override）一律退回平价，绝不当成高峰时段。

管理员可用 `usageStats/syncPrices` Remote 手动跑一次；每次结果（写入/跳过/未匹配/无变化的条数、上次运行时间、错误）随快照返回在 `priceSync` 字段。
