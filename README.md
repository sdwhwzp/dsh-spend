...
- Kimi 编程套餐（`kimi-coding`）：[官方定价页](https://platform.kimi.com/docs/pricing/chat-k3)（2026-09-09 复核）。K3 单一档、无上下文分级，每百万 token 缓存命中 ¥2 / 未命中 ¥20 / 输出 ¥100，`kimi-for-coding` 与 `k3-256k` 同价。该 provider 此前无价表，两个模型都回退到默认价。
