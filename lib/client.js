...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const USAGE_STATS_REMOTE = {
			package: "dsh-spend",
			descriptors: ["query", "sessionCost", "catalogPricing", "savePricing", "deletePricing"].map((method) => ({
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		function makeWidgetBoundary() {
			...
			componentDidCatch(error, info) {
				...
				console.error("[dsh-spend] widget render failed", error, info?.componentStack);
			}
			render() {
		...
		async function apply(ctx) {
			...
			ctx.effect(() => {
				...
				root.render(react_jsx_runtime.jsx(WidgetBoundary, { t, children: react_jsx_runtime.jsx(UsageStatsWidget, { t, query, pricingApi }) }));
				return () => {
					root.unmount();
					container.remove();
				};
