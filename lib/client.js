 ...
 *
 * Data flows through the standard Remote gateway: the host service is
 * discovered by its `typertRemote` binding and Remote method markers, and the
 * browser resolves `remote.usageStats` through an exact nested-service injection after
 * mounting the contribution. The widget renders through its own
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const parseJson = (value, seen = new Set()) => {
			...
			if (Array.isArray(value)) {
				...
				if (prototype !== Object.prototype && prototype !== null) throw new TypeError("value must be a plain JSON object");
				for (const key of Reflect.ownKeys(value)) {
					if (typeof key !== "string") throw new TypeError("JSON object keys must be strings");
					parseJson(value[key], seen);
				}
			}
			seen.delete(value);
			return value;
		};
		const jsonSchema = { parse: (value) => parseJson(value) };
		const requestSchema = {
			parse: (value) => {
				parseJson(value);
		...
		const requestCodec = { mode: "strict", typeSymbol: "dsh-spend#JsonRequest", schema: requestSchema };
		const resultCodec = { mode: "strict", typeSymbol: "dsh-spend#JsonResult", schema: jsonSchema };
		const USAGE_STATS_REMOTE = {
			package: "dsh-spend",
			descriptors: ["query", "sessionCost", "catalogPricing", "savePricing", "deletePricing", "reportRenderFailure", "syncPrices"].map((method) => ({
				id: `dsh-spend#usageStats/${method}`,
				service: "usageStats",
				namespace: "usageStats",
				method,
				invocation: { kind: "direct" },
				parameters: [{ name: "request", wire: "request", source: "json", codec: requestCodec }],
				result: resultCodec,
			})),
		};
		...
		const en = {
			...
			"pricing.promotion": "limited-time rate",
...
.dsu-pill{cursor:grab;touch-action:none;user-select:none}
.dsu-dragging .dsu-pill{cursor:grabbing;box-shadow:0 10px 28px rgba(0,0,0,.24)}
.dsu-dragging{transition:none}
...
.dsu-popLive{margin-top:10px;border-top:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.14));padding-top:8px;display:flex;flex-direction:column;gap:4px;font-size:11px;line-height:16px}
.dsu-popLiveRow{display:flex;align-items:center;justify-content:space-between;gap:10px;color:var(--dsw-alias-label-secondary,#57606a);min-width:0}
.dsu-popLiveRow b{font-weight:600;color:var(--dsw-alias-label-primary,#24292f);text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
`;
		...
		//#region data
		/**
		 * Mount and resolve the authenticated Spend Remote namespace.
		 * @param ctx - client root context (provides the root `remote` service).
		 * @returns the injected client and its mount disposer.
		 */
		async function mountUsageStatsClient(ctx) {
			const disposeRemote = await ctx.remote.$mount(USAGE_STATS_REMOTE);
			let client;
			await ctx.inject(["remote", "remote.usageStats"], (scope) => {
				client = scope.remote.usageStats;
			}).await();
			return { client, disposeRemote };
		}

		/**
		 * Invoke one method on the authenticated Spend Remote namespace.
		 * @param usageStats - injected `remote.usageStats` client.
		 * @returns a promise resolving to the aggregate snapshot.
		 */
		async function callUsageStats(usageStats, method, request) {
			const result = await usageStats[method](request);
		...
		function buildQuery(usageStats) {
			/**
			...
			return (getCwd, getPrincipalId) => async () => {
				...
				return callUsageStats(usageStats, "query", { ...(cwd ? { cwd } : {}), ...(principalId ? { principalId } : {}) });
			};
		}

		function buildPricingApi(usageStats, onChanged) {
			return {
				save: async (request) => {
					const result = await callUsageStats(usageStats, "savePricing", request);
					await onChanged();
					return result;
				},
				remove: async (request) => {
					const result = await callUsageStats(usageStats, "deletePricing", request);
					await onChanged();
					return result;
				},
			};
		...
		function installModelPriceLabels(ctx, usageStats, t) {
			let stopped = false;
			let generation = 0;
			let lastAttempt = 0;
			let pending = null;
			...
			const sync = async () => {
				...
				const catalogResponse = await ctx.remote.session.modelCatalog();
				if (catalogResponse?.ok !== true) {
					throw new Error(catalogResponse?.error?.message ?? "session.modelCatalog failed");
				}
				...
				const pricing = await callUsageStats(usageStats, "catalogPricing", { models });
		 ...
		 * activity streaks. */
		function SummaryCards({ t, totals, callCount, sessionsScanned, currency, rate, billing, budget, personalBudget, overview }) {
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
					...
					react_jsx_runtime.jsx("div", {
						...
						children: plans.map((plan) => {
							...
							if (plan.type === "code") {
								...
								// Built only when a live payload exists: a code plan whose provider
								// has no usage adapter arrives with no payload at all, and reading
								// its timestamp eagerly threw before the plan was even shown.
								const liveBody = providerUsage === null ? [] : [
								...
								const liveErrorNote = liveFailed
									? react_jsx_runtime.jsx("div", { className: "dsu-planNote", children: `${t("plan.liveError")}：${providerUsage.error}` })
									: null;
								return react_jsx_runtime.jsx("div", {
									className: "dsu-plan",
									children: [
		...
		function PricingSection({ t, rows, currency, rate, canManage, onSave, onDelete }) {
			...
			const begin = (row = null) => {
				...
				setDraft({
					...
					inputPerMillion: formatRateNum(row?.inputPerMillion ?? 0, rate),
					outputPerMillion: formatRateNum(row?.outputPerMillion ?? 0, rate),
					cacheReadPerMillion: formatRateNum(row?.cacheReadPerMillion ?? 0, rate),
					cacheWritePerMillion: formatRateNum(row?.cacheWritePerMillion ?? 0, rate),
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx("div", { className: "dsu-tableWrap", children: react_jsx_runtime.jsx(BreakdownTable, {
						...
						renderRow: (row) => [
							react_jsx_runtime.jsx("td", { children: [row.model, row.appliesTo === "default" ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", children: t("col.default") }) : null, row.custom === true ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", children: t("pricing.custom") }) : row.auto === true ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", title: t("plan.auto"), children: t("plan.auto") }) : null, row.schedule?.endsAt !== undefined ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", title: row.schedule.endsAt, children: t("pricing.promotion") }) : row.schedule !== undefined && row.schedule !== null ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", title: `${row.schedule.effectiveAt ?? ""} · ${JSON.stringify(row.schedule.peakHours ?? [])}`, children: t("pricing.peakOffpeak") }) : null, typeof row.provider === "string" ? react_jsx_runtime.jsx("span", { className: "dsu-pillTag", children: row.provider }) : null] }),
		...
		function Dashboard({ t, data, error, lastUpdated, onRefresh, onOpenDetails, cwd, onCwdChange, principalId, onPrincipalChange, onSavePricing, onDeletePricing, currency = "CNY", rate = 1, onCurrencyChange }) {
			...
			const overview = react_jsx_runtime.jsx(react_jsx_runtime.Fragment, {
				children: [
					react_jsx_runtime.jsx(SummaryCards, { t, totals, callCount: data.callCount, sessionsScanned: data.sessionsScanned, currency, rate, billing: data.billing, budget: data.budget, personalBudget: data.personalBudget, overview: data.overview }),
			...
			const details = react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx(PricingSection, { t, rows: data.pricing ?? [], currency, rate, canManage: data.canManagePricing === true, onSave: onSavePricing, onDelete: onDeletePricing }),
				],
			});
		...
		function UsageStatsWidget({ t, query, pricingApi }) {
			...
			// Where the operator dragged the widget to, persisted across reloads.
			const drag = useDraggableWidget();
			// Display currency (CNY default): persisted across reloads, applied
			// to every cost on render via the host-provided USD↔CNY rate.
			const [currency, setCurrency] = react.useState(() => {
				try {
					return localStorage.getItem("dsh-spend:currency") || "CNY";
				} catch {
					return "CNY";
				}
			});
			...
			const refresh = react.useCallback(() => {
				setLoading(true);
				// Returns the promise: savePricing/deletePricing await it so the
				// dashboard shows the saved rate rather than the pre-save snapshot.
			...
			const allowanceLabel = data?.personalBudget?.isViewer === false
				...
				: t("allowance.remaining");
			// A drag must not pop the preview open under the pointer.
			const showPreview = hovered && !expanded && !drag.dragging;
			// Compact live summary for the hover capsule: subscription quota
			// remaining (vendor-reported windows, reset times in the tooltip)
			// and wallet balances (converted with the currency switch).
			const liveSummary = (data?.plans ?? [])
				.map((plan) => {
					const label = typeof plan.label === "string" && plan.label.length > 0 ? plan.label : plan.provider;
					const usage = plan.providerUsage ?? null;
					const liveWindows = usage !== null && usage.error === null
						? ["5h", "week", "month"]
							.map((key) => {
								const w = usage.windows?.[key];
								return w !== undefined && w !== null && w.status === "ok" && typeof w.percent === "number" ? { key, ...w } : null;
							})
							.filter((row) => row !== null)
						: [];
					const balance = plan.providerBalance?.balance ?? null;
					const balanceTotal = balance !== null && typeof balance === "object" ? Number(balance.total) : Number.NaN;
					if (liveWindows.length > 0) {
						const parts = liveWindows.map((w) => `${planWindowLabel(t, w.key)} ${100 - Math.round(w.percent)}%`);
						const resets = liveWindows
							.map((w) => (w.resetsAt !== undefined ? `${planWindowLabel(t, w.key)} ${t("plan.resetAt")} ${formatResetAt(w.resetsAt)}` : null))
							.filter((text) => text !== null)
							.join(" · ");
						return { key: `live-${plan.provider}`, label, value: `${t("plan.remainingShort")} ${parts.join(" · ")}`, title: resets.length > 0 ? resets : undefined };
					}
					if (Number.isFinite(balanceTotal)) {
						return { key: `bal-${plan.provider}`, label, value: `${t("plan.liveBalance")} ${formatBalanceAmount(balance, currency, rate)}`, title: undefined };
					}
					return null;
				})
				.filter((row) => row !== null);
			...
			return react_jsx_runtime.jsx("div", {
				className: drag.dragging ? "dsu-widget dsu-dragging" : "dsu-widget",
				ref: drag.rootRef,
				style: drag.style,
				...
				children: [
						...
						? react_jsx_runtime.jsx("div", {
							...
							children: [
								...
								error !== null
									...
									react_jsx_runtime.jsx("div", { className: "dsu-popItem", children: [react_jsx_runtime.jsx("span", { className: "dsu-popLabel", children: t("summary.cost") }), react_jsx_runtime.jsx("span", { className: "dsu-popValue isCost", children: formatCost(cost, currency, rate) })] }),
												...
												(todayRow === undefined ? null : react_jsx_runtime.jsx("div", { className: "dsu-popItem", children: [react_jsx_runtime.jsx("span", { className: "dsu-popLabel", children: t("summary.today") }), react_jsx_runtime.jsx("span", { className: "dsu-popValue", children: `${formatTokensFull(todayRow.calls)} · ${formatCost(todayRow.cost, currency, rate)}` })] })),
											],
										}),
								liveSummary.length > 0
									? react_jsx_runtime.jsx("div", {
										className: "dsu-popLive",
										children: liveSummary.map((row) => react_jsx_runtime.jsx("div", { className: "dsu-popLiveRow", title: row.title ?? undefined, children: [react_jsx_runtime.jsx("span", { children: row.label }), react_jsx_runtime.jsx("b", { children: row.value })] }, row.key)),
									})
									: null,
								react_jsx_runtime.jsx("div", {
									...
									children: [
										...
										react_jsx_runtime.jsx(CurrencySwitch, { currency, onChange: changeCurrency, t }),
					...
					react_jsx_runtime.jsx("button", {
						type: "button",
						className: "dsu-pill",
						title: `${t("widget.hint")} · ${t("widget.drag")}`,
						...drag.dragProps,
						...
						children: [
							...
							react_jsx_runtime.jsx("span", { className: "dsu-pillCost", children: totals === undefined ? "…" : formatCost(cost, currency, rate) }),
		...
		//#region plugin body
		/** Where a dragged widget's corner is remembered. */
		const POSITION_KEY = "dsh-spend:position";
		/** Distance the pointer must travel before a press counts as a drag rather than a click. */
		const DRAG_THRESHOLD = 4;
		/** Kept on screen by this much even if the window shrinks around the widget. */
		const EDGE_MARGIN = 8;

		/** Read the remembered corner offsets, or the stylesheet's default corner. */
		function storedPosition() {
			try {
				const raw = globalThis.localStorage?.getItem(POSITION_KEY);
				const parsed = raw === null || raw === undefined ? null : JSON.parse(raw);
				...
				// No storage, or a value written by an older/other build.
			}
			return { right: 20, bottom: 20 };
		}

		/** Hold a corner inside the viewport, leaving the widget's own size room. */
		function clampPosition(position, size) {
			const width = globalThis.innerWidth ?? 0;
			const height = globalThis.innerHeight ?? 0;
			const maxRight = Math.max(EDGE_MARGIN, width - (size?.width ?? 0) - EDGE_MARGIN);
			const maxBottom = Math.max(EDGE_MARGIN, height - (size?.height ?? 0) - EDGE_MARGIN);
			return {
		...
		/**
		 * Let the operator move the widget and keep it where they put it.
		 *
		 * The widget sits over the product's own UI, and a fixed bottom-right
		 * corner covers whatever the page puts there. Dragging measures from the
		 * bottom-right corner, matching the `right`/`bottom` anchoring the
		 * stylesheet already uses, so the widget keeps its distance from that
		 * corner when the window resizes instead of drifting off screen.
		 *
		 * The gesture rides on the pill: a press that never travels past
		 * `DRAG_THRESHOLD` stays a click and still opens the dashboard, so moving
		 * the widget costs the operator nothing when they only meant to open it.
		 * @returns `{ style, dragProps, dragging }` for the widget root and its pill.
		 */
		function useDraggableWidget() {
			const [position, setPosition] = react.useState(storedPosition);
			const [dragging, setDragging] = react.useState(false);
			const rootRef = react.useRef(null);
			// The live corner during a drag: pointer handlers must not read a
			// state value captured when the gesture started.
			...
			const sizeOf = () => {
				...
				return rect === undefined ? undefined : { width: rect.width, height: rect.height };
			};

			// A window narrower than the remembered corner would put the widget
			// out of reach; pull it back in instead.
			react.useEffect(() => {
				const onResize = () => setPosition((current) => clampPosition(current, sizeOf()));
				onResize();
			...
			const onPointerDown = (event) => {
				// Primary button only; a right-click or a second touch point must
				// not start a drag.
			...
			const onPointerMove = (event) => {
				...
				// Rightward and downward pointer travel SHRINKS a right/bottom
				// offset, hence the inverted deltas.
				...
				setPosition(clampPosition({ right: state.origin.right + dx, bottom: state.origin.bottom + dy }, sizeOf()));
			};
			...
			const endDrag = (event) => {
				...
				setDragging(false);
				try {
					globalThis.localStorage?.setItem(POSITION_KEY, JSON.stringify(positionRef.current));
				} catch {
					// Private mode etc.: the move still holds for this visit.
				}
			};

			// A press that turned into a drag must not also open the dashboard.
		...
		/** Required services: Spend/model Remote namespaces and locale. */
		const inject = [
			"locale",
			"remote",
			"remote.session"
		 ...
		 */
		/**
		 * Keep a render failure inside the widget.
		 *
		 * Without a boundary an uncaught render error unmounts the whole root and
		 * the widget's container is left empty, which reads to the operator as the
		 * widget vanishing on click and leaves no evidence behind. The message and
		 * component stack render in place instead, and reload restores the widget
		 * without a page refresh.
		 */
		function makeWidgetBoundary() {
			return class WidgetBoundary extends react.Component {
			constructor(props) {
				super(props);
			...
			componentDidCatch(error, info) {
				...
				console.error("[dsh-spend] widget render failed", error, componentStack);
				// The console has repeatedly failed to carry this back intact, so
				// send it to the server log too. Reporting is best-effort: a
				// failed report must not replace the error it was describing.
				try {
					this.props.report?.({
					...
					// The report path itself threw; the rendered message below is
					// still the operator's copy of the failure.
				}
			}
			render() {
				...
				return react_jsx_runtime.jsx("div", {
					...
					children: react_jsx_runtime.jsx("div", {
						...
						style: { maxWidth: "42em" },
						children: [
							react_jsx_runtime.jsx("div", { className: "dsu-popTitle", children: "dsh-spend 渲染失败" }),
							react_jsx_runtime.jsx("div", { className: "dsu-error", style: { whiteSpace: "pre-wrap", fontSize: "11px" }, children: [
								String(error && error.stack ? error.stack : error),
								this.state.stack.length > 0 ? "\n\n" + this.state.stack.split("\n").slice(0, 8).join("\n") : "",
							] }),
							react_jsx_runtime.jsx("button", {
								type: "button", className: "dsu-button",
		...
		async function apply(ctx) {
			const { client: usageStats, disposeRemote } = await mountUsageStatsClient(ctx);
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-spend: dictionaries");
			ctx.effect(() => injectStyles(), "dsh-spend: styles");
			const t = ctx.locale.bind(NS);
			const WidgetBoundary = makeWidgetBoundary();
			const reportRenderFailure = (payload) => usageStats.reportRenderFailure(payload);
			const query = buildQuery(usageStats);
			const modelPrices = installModelPriceLabels(ctx, usageStats, t);
			ctx.effect(() => () => { modelPrices.dispose(); }, "dsh-spend: model price labels");
			const pricingApi = buildPricingApi(usageStats, modelPrices.refresh);
			ctx.effect(() => {
				const container = document.createElement("div");
				container.id = "dsh-spend-widget";
				document.body.appendChild(container);
				const root = react_dom_client.createRoot(container);
				root.render(react_jsx_runtime.jsx(WidgetBoundary, { t, report: reportRenderFailure, children: react_jsx_runtime.jsx(UsageStatsWidget, { t, query, pricingApi }) }));
				return () => {
					root.unmount();
					container.remove();
				};
