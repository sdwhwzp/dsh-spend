...
`;
		...
		function buildPricingApi(ctx) {
			return {
				save: (request) => callUsageStats(ctx, "savePricing", request),
				remove: (request) => callUsageStats(ctx, "deletePricing", request),
			};
		}
		//#endregion
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		function apply(ctx) {
			...
			const pricingApi = buildPricingApi(ctx);
			ctx.effect(() => {
