 ...
 * browser calls it through `ctx.remote.usageStats`. The widget renders through its own
...
`;
		 ...
		 * @param ctx - client root context (provides `remote.usageStats`).
		 * @returns a promise resolving to the aggregate snapshot.
		 */
		async function callUsageStats(ctx, method, request) {
			const result = await ctx.remote.usageStats[method](request);
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		async function apply(ctx) {
			await ctx.remote.$mount(USAGE_STATS_REMOTE);
			ctx.inject(["remote.usageStats"], (scope) => {
				scope.effect(() => scope.locale.register(NS, { zh, en }), "dsh-spend: dictionaries");
				scope.effect(() => injectStyles(), "dsh-spend: styles");
				const t = scope.locale.bind(NS);
				const query = buildQuery(scope);
				const modelPrices = installModelPriceLabels(scope, t);
				scope.effect(() => () => { modelPrices.dispose(); }, "dsh-spend: model price labels");
				const pricingApi = buildPricingApi(scope, modelPrices.refresh);
				scope.effect(() => {
