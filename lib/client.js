...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		function UsageStatsWidget({ t, query, pricingApi }) {
			...
			const refresh = react.useCallback(() => {
				...
				// Captured with the cwd so an in-flight request keeps one identity.
				const requestedPrincipalId = principalIdRef.current || null;
				if (refreshInFlight.current !== null) return;
				setLoading(true);
				const request = query(() => requestedCwd, () => requestedPrincipalId)();
