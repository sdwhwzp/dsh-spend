...
import { buildStats, computeSignature, FOLD_VERSION, localDay, normalizeProviderUsage, pricingRows, resolvePrice, scanSessions } from "./stats.js";
import {
...
const priceFields = () => ({
  ...
  // One auxiliary web search dispatch. The provider bills a search as ordinary
  // tokens on the serving model and the session log records only the request,
  // never the response usage, so the dispatch is the sole countable unit and
  // this rate is the deployment's average cost for one. Left at 0 the searches
  // are counted and shown but contribute no cost.
  searchPerCall: z.number().min(0).default(0),
});
...
/** How far a `parentSession` chain is followed before it is treated as a cycle. */
const MAX_SESSION_DEPTH = 64;

/**
 * Child-to-parent session links, restricted to trees the caller can see.
 *
 * A chain is kept whole as soon as any session on it carries the caller's own
 * calls. Membership cannot be decided per session: a parent that only
 * delegated bills nothing, and so does a fork that ran nothing of its own
 * beyond the history it inherited — dropping either would cut the sessions
 * around it off from their root and leave a task priced in pieces. A chain
 * with no visible session on it belongs to another account and is dropped
 * rather than disclosed.
 * @param sessions - every scanned session's metadata.
 * @param visibleSessionIds - ids of sessions the caller's calls appear in.
 * @returns a plain child-id to parent-id map.
 */
export function parentLinksFor(sessions, visibleSessionIds) {
  const all = new Map();
  for (const session of sessions ?? []) {
    if (typeof session?.id === "string" && typeof session.parentSession === "string") all.set(session.id, session.parentSession);
  }
  const links = {};
  for (const id of all.keys()) {
    const chain = [id];
    let at = all.get(id);
    for (let depth = 0; at !== undefined && depth < MAX_SESSION_DEPTH && !chain.includes(at); depth += 1) {
      chain.push(at);
      at = all.get(at);
    }
    if (!chain.some((node) => visibleSessionIds.has(node))) continue;
    for (const node of chain) {
      const parent = all.get(node);
      if (parent !== undefined) links[node] = parent;
    }
  }
  return links;
}
...
/**
 * Every session id in the same delegation tree as `sessionId`: its root, found
 * by walking `parentSession` up, and everything descending from that root.
 *
 * The whole tree rather than the descendants alone, because a task's session
 * is not always its tree's root — a resumed conversation is recorded as a
 * child of the one it continues, and its delegated work hangs off the
 * original. Asking either for "this session's spend" must answer with the same
 * task.
 * @param sessionId - the session asked about.
 * @param parents - child-to-parent links, as `parentLinksFor` builds them.
 * @returns the set of session ids in the tree, `sessionId` included.
 */
export function sessionTreeOf(sessionId, parents) {
  let root = sessionId;
  for (let depth = 0; depth < MAX_SESSION_DEPTH; depth += 1) {
    const up = parents[root];
    if (up === undefined || up === root) break;
    root = up;
  }
  const tree = new Set([sessionId, root]);
  // A child joins once its own chain is known to reach the root, so the pass
  // repeats until nothing new is added — the links carry no child index.
  for (let pass = 0; pass < MAX_SESSION_DEPTH; pass += 1) {
    let grew = false;
    for (const [child, parent] of Object.entries(parents)) {
      if (!tree.has(child) && tree.has(parent)) {
        tree.add(child);
        grew = true;
      }
    }
...
const PRICE_FIELDS = ["inputPerMillion", "outputPerMillion", "cacheReadPerMillion", "cacheWritePerMillion", "searchPerCall"];
...
export function normalizePricingOverride(request, usdCnyRate, now = Date.now()) {
  ...
  for (const field of PRICE_FIELDS) {
    // Rates that predate `searchPerCall` omit it, and absent means unpriced.
    // Every other rate stays required so a mistyped row is rejected rather
    // than silently zeroed.
    const value = field === "searchPerCall" ? request[field] ?? 0 : request[field];
...
function scalePriceFields(row, factor) {
  ...
  if (row.schedule !== undefined) {
    result.schedule = {
      ...
      // A republished table carries its tiers inside `phases`; without this the
      // display would convert every other rate and leave these in USD.
      ...row.schedule.phases === undefined ? {} : {
        phases: row.schedule.phases.map((phase) => ({
          ...phase,
          ...phase.peak === undefined ? {} : { peak: scalePriceFields(phase.peak, factor) },
          ...phase.offPeak === undefined ? {} : { offPeak: scalePriceFields(phase.offPeak, factor) },
        })),
      },
    };
  }
...
/**
 * Install the periodic network price sync owned by the plugin lifecycle.
 *
 * The first pass runs on the interval rather than at mount: a Host that is
 * still starting has no session scan to name the models worth pricing, and a
 * catalog fetch must never sit between the plugin and readiness.
 * @param ctx - the plugin context.
 * @param service - the usage-stats service exposing `syncPricesNow`.
 * @param intervalHours - hours between passes.
 * @param timers - timer source, injectable for tests.
 * @returns nothing; the effect's disposer clears the timer.
 */
export function registerPriceSync(ctx, service, intervalHours, timers = globalThis) {
  const hours = typeof intervalHours === "number" && Number.isFinite(intervalHours) && intervalHours > 0
    ? intervalHours
    : 24;
  ctx.effect(() => {
    let disposed = false;
    const sync = () => {
      void service.syncPricesNow().catch((error) => {
        if (!disposed) console.warn("[dsh-spend] 联网价格同步失败：", error);
      });
    };
    const timer = timers.setInterval(sync, hours * 60 * 60 * 1_000);
    timer?.unref?.();
    return () => {
      disposed = true;
      timers.clearInterval(timer);
    };
  }, "dsh-spend: network price sync");
}
...
let UsageStatsService = (() => {
  ...
  return class UsageStatsService extends _classSuper {
    static {
      for (const name of ["query", "sessionCost", "catalogPricing", "savePricing", "deletePricing", "reportRenderFailure", "syncPrices"]) {
    ...
    static Config = z.object({
      ...
      priceVersion: z.string().default("2026-09-11"),
      requirePricedModel: z.object({
      ...
      priceSync: z.object({
        ...
        source: z.string().default(DEFAULT_PRICE_SOURCE),
        endpoint: z.string().default(DEFAULT_PRICE_ENDPOINT),
        timeoutMs: z.number().min(1000).default(20000),
        fillUnpricedOnly: z.boolean().default(true),
    ...
    constructor(ctx, config = {}) {
      ...
      this.requirePricedModel = {
        enabled: config.requirePricedModel?.enabled === true,
        // `provider/model`, or a bare model id to exempt it on every route.
        allowModels: new Set((config.requirePricedModel?.allowModels ?? []).map((entry) => String(entry).toLowerCase())),
      };
      this.priceSync = {
        enabled: config.priceSync?.enabled === true,
        intervalHours: config.priceSync?.intervalHours ?? 24,
        source: config.priceSync?.source ?? DEFAULT_PRICE_SOURCE,
        endpoint: config.priceSync?.endpoint ?? DEFAULT_PRICE_ENDPOINT,
        timeoutMs: config.priceSync?.timeoutMs ?? 20000,
        fillUnpricedOnly: config.priceSync?.fillUnpricedOnly !== false,
        providers: (config.priceSync?.providers ?? []).map((provider) => normalizeProvider(provider)),
      };
      /** Outcome of the newest sync pass, surfaced in the snapshot. */
      this.priceSyncState = { enabled: this.priceSync.enabled, lastRunAt: null, lastError: null, written: 0, source: this.priceSync.source };
      ...
      if (this.priceSync.enabled) registerPriceSync(ctx, this, this.priceSync.intervalHours);
      if (this.requirePricedModel.enabled) {
        // Refuse before the adapter is reached, not after: a model nobody can
        // price spends real money that no report will ever attribute. The
        // listener is prepended so it runs ahead of retry and routing, and
        // throwing here fails the call rather than yielding a partial stream.
        ctx.effect(() => ctx.on("llm/stream", (options, next) => {
          const refusal = this.unpricedRefusal(options?.provider, options?.model);
          if (refusal !== undefined) throw new Error(refusal);
          return next();
        }, { prepend: true }), "dsh-spend: refuse unpriced models");
      }
    ...
    /**
     * Effective DeepSeek V4 peak and off-peak rates at one instant, for a host
     * consumer that prices totals already bucketed by period rather than one
     * call at a time (dsh-context's stats board). Overrides, configured rows
     * and the knowledge base resolve exactly as the ledger resolves them, so
     * the board can never quote a rate the ledger would not charge.
     *
     * The two instants are the same Beijing day as `atTime`, moved to a
     * weekday when the peak table restricts itself to weekdays, so a Saturday
     * caller still sees what a peak hour costs.
     * @param atTime - the moment whose published table applies.
     * @returns rates per million tokens in both currencies, keyed
     *   family → period → `{ hit, miss, out }`.
     */
    costRatesAt(atTime = Date.now()) {
      const rows = this.pricingFor([{ provider: "deepseek-official" }]);
      const beijing = new Date(atTime + 8 * 3600_000);
      const atHour = (hour, weekday) => {
        const day = new Date(beijing);
        day.setUTCHours(hour, 0, 0, 0);
        // 0 is Sunday and 6 Saturday; step back to the nearest Friday.
        if (weekday && (day.getUTCDay() === 0 || day.getUTCDay() === 6)) {
          day.setUTCDate(day.getUTCDate() - (day.getUTCDay() === 0 ? 2 : 1));
        }
        return day.getTime() - 8 * 3600_000;
      };
      const triple = (model, when) => {
        const row = resolvePrice(model, "deepseek-official", rows, this.defaultPricing, when);
        return row === undefined ? undefined : {
          hit: row.cacheReadPerMillion ?? 0,
          miss: row.inputPerMillion ?? 0,
          out: row.outputPerMillion ?? 0,
        };
      };
      const usd = {};
      for (const [family, model] of [["flash", "deepseek-v4-flash"], ["pro", "deepseek-v4-pro"]]) {
        const peak = triple(model, atHour(10, true));
        const off = triple(model, atHour(20, false));
        if (peak !== undefined && off !== undefined) usd[family] = { peak, off };
      }
      if (Object.keys(usd).length === 0) return undefined;
      const rate = this.usdCnyRate;
      const scale = (t) => ({ hit: t.hit * rate, miss: t.miss * rate, out: t.out * rate });
      const cny = {};
      for (const [family, tiers] of Object.entries(usd)) cny[family] = { peak: scale(tiers.peak), off: scale(tiers.off) };
      return { usd, cny, usdCnyRate: rate, atTime };
    }
    ...
    pricingFor(calls) {
      ...
      // Network observations sit BELOW everything a person put in place: an
      // administrator's override and a hand-verified knowledge row both keep
      // their rate, and a synced row only reaches a model neither of them
      // prices -- unless the provider is on the full-sync list, where the
      // operator has asked the catalog to lead.
      const knowledgeKeys = new Set(automatic.map((rate) => `${rate.provider}:${rate.model}`));
      const fullSync = new Set(this.priceSync.providers);
      const synced = [];
      for (const row of this.ledger.syncedPrices().values()) {
        const key = `${row.provider}:${row.model}`;
        if (explicit.has(key) || explicit.has(`*:${row.model}`)) continue;
        if (knowledgeKeys.has(key) && !fullSync.has(normalizeProvider(row.provider))) continue;
        const rate = syncedRateRow(row);
        if (rate === undefined) continue;
        synced.push(rate);
      }
      const syncedKeys = new Set(synced.map((rate) => `${rate.provider}:${rate.model}`));
      return [...explicitRows, ...synced, ...automatic.filter((rate) => !syncedKeys.has(`${rate.provider}:${rate.model}`))];
    }

    /**
     * Why this exact model may not be called, or undefined when it may.
     *
     * A model with no rate row of its own prices at the deployment's default
     * row — a guess that spends real money no report can attribute — so a
     * deployment that has turned this on refuses the call outright rather
     * than billing it at a number nobody chose.
     *
     * Resolution is the same one the reports use, so a model is callable
     * exactly when it is reportable: an administrator's override, a verified
     * knowledge row, or a synced row all qualify, and an explicit
     * `allowModels` entry exempts a route that is genuinely free.
     * @param provider - the request's provider route.
     * @param model - the request's model id.
     * @returns the refusal message, or undefined to allow the call.
     */
    unpricedRefusal(provider, model) {
      if (typeof provider !== "string" || typeof model !== "string") return undefined;
      const exempt = this.requirePricedModel.allowModels;
      if (exempt.has(model.toLowerCase()) || exempt.has(`${provider}/${model}`.toLowerCase())) return undefined;
      // `defaultPricing` is the fallback `resolvePrice` returns when nothing
      // matched, so identity against it is exactly "this model has no row".
      const rows = this.pricingFor([{ provider }]);
      const resolved = resolvePrice(model, provider, rows, this.defaultPricing, Date.now());
      if (resolved !== this.defaultPricing) return undefined;
      return `dsh-spend: ${provider}/${model} 没有定价，本部署不允许调用未定价的模型。`
        + "请在 Spend 面板为它设置内部价格，或把它加入 requirePricedModel.allowModels。";
    }

    /**
     * Run one network price sync pass.
     *
     * Every write is an APPEND: a model's observed price becomes a new phase
     * stamped with this moment, so calls already priced under an earlier phase
     * keep that price and no history is recomputed.
     * @returns a summary of the pass (written / skipped / unmatched / unchanged counts).
     */
    async syncPricesNow() {
      const observedAt = Date.now();
      try {
        const scanned = await scanSessions(this.sessionsRoot, [], this.scanCache);
        // Only models the deployment actually calls are worth a row.
        const wanted = [...new Map(scanned.calls
          .filter((call) => typeof call.provider === "string" && typeof call.model === "string")
          .map((call) => [`${normalizeProvider(call.provider)}:${call.model}`, {
            provider: normalizeProvider(call.provider),
            model: call.model,
          }])).values()];
        const priced = new Set();
        for (const { provider, model } of wanted) {
          if (autoRatesFor(provider).some((rate) => rate.model === model)) priced.add(`${provider}:${model}`);
        }
        const overridden = new Set(this.ledger.pricingOverrides().map((row) => `${normalizeProvider(row.provider)}:${row.model}`));
        const index = indexCatalog(await fetchCatalog(fetch, this.priceSync.endpoint, this.priceSync.timeoutMs));
        const plan = planSync({
          wanted,
          index,
          stored: this.ledger.syncedPrices(),
          priced,
          overridden,
          providers: this.priceSync.providers,
          fillUnpricedOnly: this.priceSync.fillUnpricedOnly,
          observedAt,
        });
        for (const write of plan.writes) {
          this.ledger.saveSyncedPrice({ ...write, source: this.priceSync.source });
        }
        // A new rate only reaches the dashboard once the snapshot cache drops.
        if (plan.writes.length > 0) this.cache = new Map();
        this.priceSyncState = {
          enabled: this.priceSync.enabled,
          source: this.priceSync.source,
     ...
     */
    /**
     * The priced cost of one session, for a card that shows a session's spend
     * without keeping its own price table (dsh-context's Context card). Covers
     * every model the ledger prices, not just one provider's.
     *
     * The answer carries the billed TOKEN USAGE beside the money, at the
     * session level and per model, because that usage is what the money was
     * computed from: every row here is priced by tokens, so a consumer that
     * shows the split shows the same quantity the ledger charged on rather
     * than a call tally that no rate applies to.
     *
     * `cost` is stated in `currency`, the deployment's server-side BASE
     * currency, which is not what the dashboard prints: the browser picks a
     * display currency (CNY by default) and converts at the host's USD-CNY
     * quote. `rates` carries that quote so a consumer converts to the same
     * figure the dashboard shows instead of printing the base amount under
     * the base symbol.
     *
     * The answer covers the session's whole DELEGATION TREE, not the one row
     * keyed by its id: workflow members, subagents and a resumed conversation
     * all bill into their own child sessions, so the row alone reports a
     * fraction of what the task cost and names only the models the parent
     * itself called. `sessions` says how many rows were folded.
     * @param request - `{ sessionId }`.
     * @returns the tree's cost, or a null cost when the caller may not read
     *   that session or nothing in it priced.
     */
    async sessionCost(request) {
      return this.sessionCostForPrincipal(request, this.ctx.typertGateway.currentPrincipal());
    }

    /**
     * @param request - `{ sessionId }`.
     * @param principal - the authenticated caller.
     * @returns cost, base currency, the display FX quote, billed token usage,
     *   the folded session count and the per-model split for the tree.
     */
    async sessionCostForPrincipal(request, principal) {
      if (principal === undefined) throw new Error("authenticated principal required");
      const sessionId = typeof request?.sessionId === "string" ? request.sessionId : "";
      if (sessionId === "") throw new Error("sessionId required");
      // The snapshot is already scoped to the caller, so a session owned by
      // another principal has no row here and reads as an unpriced session.
      const snapshot = await this.snapshotFor(principal, null, null);
      const tree = sessionTreeOf(sessionId, snapshot.sessionParents ?? {});
      // The per-model rows, not the per-session ones: `bySession` is capped at
      // `maxSessions` and a tree of delegated work runs past that cap, while
      // the model rows are complete.
      const rows = (snapshot.bySessionModel ?? []).filter((candidate) => tree.has(candidate.sessionId));
      // The four billed streams, each as the aggregation counted it. Reasoning
      // tokens stay out: the provider reports them INSIDE the output count, so
      // adding them would bill the same tokens twice.
      const STREAMS = ["inputTokens", "outputTokens", "cacheReadTokens", "cacheWriteTokens"];
      const emptyUsage = () => Object.fromEntries(STREAMS.map((name) => [name, 0]));
      const addUsage = (target, bucket) => {
        for (const name of STREAMS) target[name] += bucket?.[name] ?? 0;
      };
      const totals = { cost: 0, calls: 0, ...emptyUsage() };
      const byKey = new Map();
      for (const row of rows) {
        totals.cost += row.cost ?? 0;
        totals.calls += row.calls ?? 0;
        addUsage(totals, row);
        // One model reached through one provider is one line, however many
        // sessions of the tree called it.
        const key = `${row.provider ?? ""}\u0000${row.model ?? ""}`;
        let entry = byKey.get(key);
        if (entry === undefined) {
          entry = { model: row.model, provider: row.provider, calls: 0, cost: 0, ...emptyUsage() };
          byKey.set(key, entry);
        }
      ...
      return {
        sessionId,
        cost: rows.length === 0 ? null : totals.cost,
        calls: totals.calls,
        sessions: new Set(rows.map((row) => row.sessionId)).size,
        currency: this.currency,
        rates: { USD: rates.USD ?? 1, CNY: rates.CNY ?? 0, source: rates.source ?? "fixed" },
        inputTokens: totals.inputTokens,
        outputTokens: totals.outputTokens,
        cacheReadTokens: totals.cacheReadTokens,
        cacheWriteTokens: totals.cacheWriteTokens,
        byModel: [...byKey.values()].sort((a, b) => b.cost - a.cost || b.calls - a.calls),
      };
    }
    ...
    /**
     * Record a browser render failure the widget's boundary caught.
     *
     * A render error unmounts the widget in the operator's browser and leaves
     * its evidence only in that browser's console, where it has repeatedly
     * failed to reach a maintainer intact. Writing it to the server log puts
     * the message, stack and component stack where the deployment's logs
     * already are. The report is diagnostic only: nothing reads it back, and a
     * caller that sends garbage gets its garbage truncated and logged.
     * @param request - `{ message, stack, componentStack }`, each optional.
     * @returns `{ recorded: true }` once written.
     */
    async reportRenderFailure(request) {
      const principal = this.ctx.typertGateway.currentPrincipal();
      if (principal === undefined) throw new Error("authenticated principal required");
      const clip = (value, limit) => (typeof value === "string" ? value : "").slice(0, limit);
      const who = `${principal.source}:${principal.id}`;
      console.error(
        `[dsh-spend] 浏览器渲染失败 account=${who}\n  message: ${clip(request?.message, 400)}\n  stack:\n${clip(request?.stack, 2000)}\n  componentStack:\n${clip(request?.componentStack, 2000)}`,
      );
      return { recorded: true };
    }

    /**
     * Run one price sync on demand.
     *
     * Writing prices is an administrator action, the same gate the manual
     * price editor uses.
     * @param request - unused.
     * @returns the pass summary.
     */
    async syncPrices(request) {
      assertPricingAdministrator(this.ctx.typertGateway.currentPrincipal());
      if (!this.priceSync.enabled) throw new Error("price sync is disabled; set priceSync.enabled");
      return this.syncPricesNow();
    }
    ...
    /**
     * The caller's dashboard snapshot, from cache when one request already
     * built it. Every consumer shares this one path so a second reader never
     * costs a second scan, and the snapshot is already scoped to the caller:
     * `compute` filters the calls before aggregating, so a session another
     * principal owns is simply absent rather than hidden afterwards.
     * @param principal - the authenticated caller.
     * @param cwd - workspace filter, or null for every workspace.
     * @param principalId - the account an administrator is inspecting.
     * @returns the aggregated snapshot.
     */
    async snapshotFor(principal, cwd, principalId) {
      const live = this.ctx.sessions.list().map((session) => ({
        id: session.id,
    ...
    async queryForPrincipal(request, principal) {
      ...
      const principalId = principal.role === "admin" && typeof request?.principalId === "string" && request.principalId.length > 0
        ? request.principalId
        : null;
      const snapshot = await this.snapshotFor(principal, cwd, principalId);
        ...
        try {
          ...
          if (entries && typeof entries === "object" && !Array.isArray(entries)) {
            for (const [key, val] of Object.entries(entries)) {
              // A row this build would fold differently is dropped rather
              // than trusted: it was written by another fold version and no
              // file change will ever invalidate it.
              if (
                val && typeof val === "object"
                && val.fold === FOLD_VERSION
    ...
    async compute(signature, live, cwd, principal, principalId) {
      ...
      // Delegated work (a workflow member, a subagent) and a resumed
      // conversation each run in their OWN session carrying `parentSession`,
      // so one task's spend is spread over a tree of session rows. The
      // snapshot names the child-to-parent links so a consumer can add a
      // tree up. Only links inside a tree the caller can already see are
      // named: a session graph belonging to another account is dropped
      // rather than disclosed.
      const sessionParents = parentLinksFor(scanned.sessions, visibleSessionIds);
