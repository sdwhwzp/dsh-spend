 ...
 *
 * A session is kept when it carries the caller's own calls or when it is an
 * ancestor of one that does — an ancestor may itself hold no calls (a parent
 * that only delegated) and dropping it would cut the chain its children reach
 * the root through.
...
export function parentLinksFor(sessions, visibleSessionIds) {
  ...
  const keep = new Set(visibleSessionIds);
  for (const id of visibleSessionIds) {
    let at = all.get(id);
    for (let depth = 0; at !== undefined && depth < MAX_SESSION_DEPTH; depth += 1) {
  ...
  for (const [id, parent] of all) {
    if (keep.has(id)) links[id] = parent;
  }
  return links;
}
