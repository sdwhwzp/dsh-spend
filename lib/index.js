...
let UsageStatsService = (() => {
  ...
  return class UsageStatsService extends _classSuper {
     ...
     * @returns the session's cost, or a null cost when the caller may not read
     ...
     *   and the per-model split for that session.
     */
    async sessionCostForPrincipal(request, principal) {
      ...
      // The same quote the dashboard renders with; `snapshotFor` above has
      // already warmed its 6h cache, so this costs no second lookup.
      const rates = await this.getRates();
      const row = (snapshot.bySession ?? []).find((candidate) => candidate.sessionId === sessionId);
      ...
      return {
        ...
        rates: { USD: rates.USD, CNY: rates.CNY, source: rates.source },
        ...usageOf(row),
        byModel,
      };
    }
