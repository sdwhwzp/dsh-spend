...
export function priceUsageMicros(call, pricing, usdCnyRate) {
  ...
  // Auxiliary searches price per dispatch against their own serving model,
  // scaled to the per-million denominator the token terms already use.
  const searchCalls = Number.isSafeInteger(call.searchCalls) && call.searchCalls > 0 ? BigInt(call.searchCalls) : 0n;
  if (searchCalls > 0n) {
    const searchRow = resolvePrice(call.searchModel ?? call.model, call.provider, pricing, undefined, atTime);
    numerator += searchCalls * scaledDecimal(searchRow?.searchPerCall ?? 0) * fx * 1_000_000n;
  }
...
export class SpendLedger {
  ...
  migrate() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS spend_synced_prices (
      ...
      CREATE TABLE IF NOT EXISTS spend_pricing_overrides (
        provider TEXT NOT NULL,
        model TEXT NOT NULL,
        ...
        search_per_call REAL NOT NULL DEFAULT 0,
        price_version TEXT NOT NULL,
        updated_at INTEGER NOT NULL,
        PRIMARY KEY (provider, model)
      );
    `);
    // Overrides stored before this rate existed keep their row; the added
    // column defaults to unpriced. SQLite has no IF NOT EXISTS for a column,
    // so a second run throws "duplicate column name" and is nothing to do.
    try {
      this.db.exec("ALTER TABLE spend_pricing_overrides ADD COLUMN search_per_call REAL NOT NULL DEFAULT 0");
    } catch (error) {
    ...
    `);
    this.selectPricingOverrides = this.db.prepare(`
      SELECT provider, model, input_per_million, output_per_million,
        cache_read_per_million, cache_write_per_million, price_version, updated_at
      FROM spend_pricing_overrides ORDER BY provider, model
    `);
    this.upsertPricingOverride = this.db.prepare(`
      INSERT INTO spend_pricing_overrides (
        provider, model, input_per_million, output_per_million,
        cache_read_per_million, cache_write_per_million, search_per_call, price_version, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(provider, model) DO UPDATE SET
        ...
        search_per_call=excluded.search_per_call,
        price_version=excluded.price_version,
        updated_at=excluded.updated_at
    `);
    this.deletePricingOverrideStatement = this.db.prepare(`
      DELETE FROM spend_pricing_overrides WHERE provider = ? AND model = ?
    `);
    this.selectSyncedPrices = this.db.prepare(`
      SELECT provider, model, source, catalog_id, observed_at, phases_json
      FROM spend_synced_prices ORDER BY provider, model
    `);
    this.upsertSyncedPrice = this.db.prepare(`
      INSERT INTO spend_synced_prices (provider, model, source, catalog_id, observed_at, phases_json)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT(provider, model) DO UPDATE SET
  ...
  /** Return administrator-managed exact provider/model prices in USD per million tokens. */
  pricingOverrides() {
    return this.selectPricingOverrides.all().map((row) => ({
      ...
      searchPerCall: row.search_per_call ?? 0,
  ...
  /** Insert or replace one administrator-managed exact price. */
  savePricingOverride(row) {
    this.upsertPricingOverride.run(
      row.provider,
      row.model,
      row.inputPerMillion,
    ...
    return this.pricingOverrides().find((candidate) => candidate.provider === row.provider && candidate.model === row.model);
  }

  /** Delete one administrator-managed exact price without changing priced history. */
  deletePricingOverride(provider, model) {
    return Number(this.deletePricingOverrideStatement.run(provider, model).changes) > 0;
  }

  /**
   * Prices observed from the network catalog, keyed `provider:model`.
   *
   * A row's `phases` accumulate: each observation that differed from the one
   * before it is appended behind its own `effectiveAt`, so the list is the
   * model's price history as this deployment watched it.
   * @returns a Map of `provider:model` to `{ provider, model, source, catalogId, observedAt, phases }`.
   */
  syncedPrices() {
    const rows = new Map();
    for (const row of this.selectSyncedPrices.all()) {
      let phases;
      try {
        phases = JSON.parse(row.phases_json);
      } catch {
        // A row written by another build, or truncated on disk: the model
        // simply reads as never synced and the next pass rewrites it.
        continue;
      }
      if (!Array.isArray(phases) || phases.length === 0) continue;
      rows.set(`${row.provider}:${row.model}`, {
        provider: row.provider,
        model: row.model,
        source: row.source,
  ...
  /**
   * Store one model's observed phase list.
   * @param row - `{ provider, model, source, catalogId, observedAt, phases }`.
   */
  saveSyncedPrice(row) {
    this.upsertSyncedPrice.run(
      row.provider,
      row.model,
      row.source,
      row.catalogId ?? null,
  ...
  ingest(call) {
    ...
    const result = this.insert.run(
      ...
      priced.priced && typeof priced.rate?.priceVersion === "string" ? priced.rate.priceVersion : this.options.priceVersion,
      this.options.fxVersion,
...
export class SpendAccountingService extends Service {
  ...
  /**
   * Record final usage from a trusted Host feature outside the Agent loop.
   * @param principal - account authenticated by the Host, never read from the usage payload.
   * @param call - canonical token counts and a stable sessionId/turn/step identity for retry deduplication.
   * @returns whether the account ledger inserted this usage sample.
   */
  recordUsage(principal, call) {
    if (principalOf(principal) === undefined) throw new Error("authenticated principal required");
    return this.ledger.ingest({ ...call, principal, final: true });
  }
