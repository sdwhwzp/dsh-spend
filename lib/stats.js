...
function createSessionFolder(meta) {
  ...
  let turnPrincipal;
  let currentTurn;
  /** Auxiliary search dispatches attributed to a sampled step, keyed `${turn}:${step}`. */
  const searches = new Map();
  /** Key of the most recently emitted sample; auxiliary events attach to it. */
  let lastSampleKey;
  ...
  const consume = (event) => {
    switch (event.type) {
      ...
      case "web/deepseek-search-llm-request": {
        // One auxiliary search dispatch. The provider bills it as ordinary
        // tokens on the serving model and logs no response usage, so the
        // dispatch is the only durable unit. It is appended while the tool
        // loop runs, which is after its step's `assistant/message` has already
        // closed that step, so it attributes to the most recently sampled
        // step rather than to an open one.
        if (lastSampleKey === undefined) break;
        const model = event.data?.body?.model;
        const seen = searches.get(lastSampleKey) ?? { calls: 0, model: undefined };
        seen.calls += 1;
        if (typeof model === "string") seen.model ??= model;
        searches.set(lastSampleKey, seen);
        break;
      }
      case "turn/start": {
      ...
      case "session/end-seed": {
        // A seeded session's log OPENS with a verbatim copy of the history it
        // was forked from, and an `inherited` end-seed marker closes that
        // copy. Those calls were already billed to the session they were
        // copied from, so counting them again bills one model call once per
        // fork of the conversation. Everything the fork actually ran follows
        // the marker.
        //
        // A marker WITHOUT the flag is one the session wrote itself, closing
        // a seed of its own, and cuts nothing. The persistence contract puts
        // the cut at the LAST inherited marker, which is what dropping
        // everything accumulated at each one arrives at.
        if (event.data?.inherited !== true) break;
        samples.clear();
        steps.clear();
        searches.clear();
        lastSampleKey = undefined;
        break;
      }
      default: {
  ...
  /** Fold the auxiliary search counts into the samples they belong to. */
  const finish = () => {
    for (const [key, seen] of searches) {
      const sample = samples.get(key);
      if (sample === undefined) continue;
      sample.searchCalls = seen.calls;
      if (seen.model !== undefined) sample.searchModel = seen.model;
    }
  ...
  for (const line of lines) {
    ...
    if (record !== null && record.type === "session" && typeof record.id === "string") {
      return {
        ...
        // A delegated session records the session that spawned it; its own
        // turns carry no principal, so ownership resolves through this link.
...
/**
 * How this build folds a log into call samples.
 *
 * `scanSessions` caches a decoded file by size and mtime, and that cache is
 * persisted across restarts — so a change to what folding COUNTS is invisible
 * to it: the files are untouched, every entry still hits, and the new rule
 * never runs on the history it was written for. Bump this whenever folding
 * changes what a sample is or which events produce one; entries stamped with
 * another value are re-read.
 *
 * 2: an `inherited` end-seed marker cuts a fork's copied history (2026-09-10).
 */
...
async function scanSessionFile(file, entry, workspace, handle) {
  ...
  try {
        ...
        if (event.type === "session") {
          ...
          if (!metaFound && typeof event.id === "string") {
            Object.assign(meta, {
              ...
              // Carries the delegation link; ownership resolves through it.
              parentSession: typeof event.parentSession === "string" ? event.parentSession : undefined,
            });
            metaFound = true;
          }
          continue;
    ...
    return { fold: FOLD_VERSION, size: handle.size, mtimeMs: handle.mtimeMs, meta, samples, decodeError: false };
  } catch {
    return { fold: FOLD_VERSION, size: handle.size, mtimeMs: handle.mtimeMs, meta: undefined, samples: [], decodeError: true };
  }
}
...
/** `session.jsonl` is generation 0; later generations are `session.v<N>.jsonl`. */
const SESSION_FILE = /^session(?:\.v(\d+))?\.jsonl\.zstd$/;

/**
 * The newest committed generation in one session directory.
 *
 * A format migration adds a version-named successor beside the file it was
 * read from and never rewrites the predecessor, so a directory holds every
 * generation it has ever had and only the highest one is current. Reading a
 * fixed name would silently stop counting each session the moment it migrates.
 *
 * @param sessionDir - one session's directory.
 * Only the compressed form is considered, which is the only one the reader
 * decodes and the only one persistence writes.
 * @returns the newest generation's path, or undefined when the directory
 *   holds no session body.
 */
async function latestGenerationIn(sessionDir) {
  let names;
  try {
...
export async function scanSessions(root, liveSessions = [], fileCache = new Map()) {
  ...
  for (const workspace of workspaces) {
    ...
    for (const entry of entries) {
      ...
      const file = await latestGenerationIn(sessionDir);
  ...
  const decoded = await mapLimit(files, SESSION_SCAN_CONCURRENCY, async ({ file, entry, workspace, handle }) => {
    ...
    if (cached !== undefined && cached.fold === FOLD_VERSION && cached.size === handle.size && cached.mtimeMs === handle.mtimeMs) return cached;
  ...
  inheritDelegatedPrincipals(merged, sessions);
  return {
    calls: merged,
    sessions,
...
/**
 * Attribute a delegated session's calls to the principal that owns the
 * session which spawned it.
 *
 * A subagent session's own turns carry no principal, so without this every
 * call a workflow delegates is unowned: invisible to its user and absent from
 * per-account accounting, while the same work run in the parent session is
 * charged normally. Ownership walks the `parentSession` chain to the nearest
 * ancestor that recorded one, and the inherited samples are marked so a
 * consumer can tell a resolved owner from a recorded one.
 *
 * @param calls - every sample from this scan, mutated in place.
 * @param sessions - session metadata carrying the delegation links.
 */
function inheritDelegatedPrincipals(calls, sessions) {
  const parentOf = new Map();
  for (const meta of sessions) {
    if (meta?.id !== undefined && meta.parentSession !== undefined) parentOf.set(meta.id, meta.parentSession);
  }
  if (parentOf.size === 0) return;
  const ownerOf = new Map();
  for (const call of calls) {
    if (call.principal !== undefined && call.sessionId !== undefined && !ownerOf.has(call.sessionId)) {
      ownerOf.set(call.sessionId, call.principal);
    }
  }
  /** Nearest recorded owner above one session, or undefined. */
  const resolve = (sessionId) => {
    const seen = new Set();
    let at = parentOf.get(sessionId);
  ...
  for (const call of calls) {
    if (call.principal !== undefined || call.sessionId === undefined) continue;
    if (!resolved.has(call.sessionId)) resolved.set(call.sessionId, resolve(call.sessionId));
    const owner = resolved.get(call.sessionId);
    if (owner === undefined) continue;
    call.principal = owner;
    call.principalInherited = true;
  }
}
...
function applySchedule(row, atTime) {
  ...
  if (Number.isFinite(endsAtMs) && atTime >= endsAtMs && schedule.after !== undefined) {
    return tierRow(row, schedule.after);
  }
  // A provider may republish its peak/off-peak table; each phase carries the
  // moment it took effect and the phases are ordered oldest first, so a call
  // prices under the table that was live at its own timestamp.
  const phases = schedule.phases ?? [schedule];
  let phase;
  for (const candidate of phases) {
    const at = candidate.effectiveAtMs ?? (typeof candidate.effectiveAt === "string" ? Date.parse(candidate.effectiveAt) : undefined);
    if (Number.isFinite(at) && atTime >= at) phase = candidate;
  }
  if (phase === undefined) return row;
  const shanghai = new Intl.DateTimeFormat("en-US", {
    timeZone: "Asia/Shanghai", hour: "2-digit", hourCycle: "h23", weekday: "short",
  }).formatToParts(new Date(atTime));
  const hour = Number(shanghai.find((part) => part.type === "hour")?.value);
  const inHours = (phase.peakHours ?? []).some(([from, to]) => hour >= from && hour < to);
  // `peakDays` restricts the peak to those weekdays; absent, every day peaks.
  const day = WEEKDAY_INDEX[shanghai.find((part) => part.type === "weekday")?.value ?? ""];
  const inDays = phase.peakDays === undefined || phase.peakDays.includes(day);
  const tier = inHours && inDays ? phase.peak : phase.offPeak;
  if (tier === undefined) return row;
  return tierRow(row, tier);
}

/** Monday-first weekday numbers for `peakDays`. */
const WEEKDAY_INDEX = { Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6, Sun: 7 };

/**
 * Overlay one schedule tier on its base row.
 * @param row - the base price row.
 * @param tier - the tier's overrides; absent fields keep the base rate.
 * @returns the effective price row.
 */
function tierRow(row, tier) {
  return {
    ...
    searchPerCall: tier.searchPerCall ?? row.searchPerCall,
  };
}
...
export function costOf(sample, pricing, defaultPricing) {
  ...
  // Auxiliary searches are served by their own model, so they price against
  // that model's row rather than the step's.
  const searchCalls = Number.isSafeInteger(sample.searchCalls) && sample.searchCalls > 0 ? sample.searchCalls : 0;
  const searchPrice = searchCalls === 0
    ? undefined
    : resolvePrice(sample.searchModel ?? sample.model, sample.provider, pricing, defaultPricing, sample.time);
  const costSearch = searchCalls * (searchPrice?.searchPerCall ?? 0);
  return {
...
export async function computeSignature(root, liveSessions = []) {
  ...
  for (const workspace of workspaces) {
    ...
    for (const entry of entries) {
      // The newest generation, which is what `scanSessions` reads. A fixed
      // `session.jsonl.zstd` matches nothing once a session has migrated, and
      // the signature then stops changing when that session's log grows.
      const file = await latestGenerationIn(join(workspaceDir, entry));
      if (file === undefined) continue;
      try {
